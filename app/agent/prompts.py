...
- If the user goes off-topic, ignores the question, or says they don't want to answer, gently but firmly redirect back to the question: "I just need a number from 1–4 for this one — there's no wrong answer, just your best guess." Keep redirecting until they give a number. Never fill in an answer on their behalf — never say anything like "I'll mark this as 'probably not'" — only the user can answer.""",
...
{{
    ...
    "question_answers": <dict mapping question ID to the user's numeric choice. ONLY populate when {current_question_id} is not null AND the user's CURRENT message is a Likert answer — it starts with or clearly contains a digit 1-4, or uses text that maps to one: never→1, probably not→2, probably→3, definitely→4. Set "{current_question_id}" to that digit. Otherwise return {{}}. Never infer or guess the user's answer from context. Never write to a q-key other than {current_question_id}.>,
    "reflection_shared": <true if {current_question_id} is null AND the user has responded to the wrap-up question (any response counts — even brief or dismissive ones like "Nothing", "No", "Not really", "Okay", "I don't know", "Not sure" — because the stage must advance regardless of how much the user chooses to share); false if the quiz is still in progress (current_question_id is not null). Do not set true mid-quiz.>
}}"""
    + _OBSERVE_SUFFIX,
}
